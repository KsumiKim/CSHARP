    ...
    public class UltimateRule : AdvancedRule
    {
        ...
        public static int RunRound(List<Player> players, int winnerNo)
        {
            ...
            {   // 컴퓨터가 승자일 때, 컴퓨터는 결과에 상관없이 판돈의 두 배를 올린다. 
                ...
                input = Randomizer.Instance.Next(1, 4);
                MultipleType multipleType = (MultipleType)input;
                Console.WriteLine($"P[{winnerNo}]는 {(int)multipleType}배를 선택하여 이번 판의 판돈이 {(int)multipleType}배 증가하였습니다.");
                // input = 2;
            }
            ...
            if (FindWinner(players).Count >= 2) // 승자가 두명이면 
            {
                ...
                Player winner = FindHighestCardHolder(players[winner0], players[winner1]);
        ...
        private static Player FindHighestCardHolder(Player player1, Player player2)
        {
            //List<Card> player1Cards = player1.getCard();
            //List<Card> player2Cards = player2.getCard();

            //if (player1Cards[0].No > player2Cards[0].No)
            //    return player1;
            //else
            //    return player2;

            if (player1[0].No > player2[0].No)
                return player1;
            else
                return player2;
